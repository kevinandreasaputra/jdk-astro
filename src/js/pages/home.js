...
export async function initializeHomePage() {
    ...
    if (document.getElementById('particle-container') && window.innerWidth >= 768) {
        import('./particles.js').then(({ initializeRetroParticles }) => {
            initializeRetroParticles();
        }).catch(e => logger.error('Failed to load particles:', e));
    }
...
export function cleanupHomePage() {
    if (window.innerWidth >= 768) {
        import('./particles.js').then(({ cleanupRetroParticles }) => {
            cleanupRetroParticles();
        }).catch(e => logger.error('Failed to cleanup particles:', e));
    }
}
